...
export default class Node {
  ...
  constructor(puzzle, greedy, uniform, goal, parent, heuristic) {
    this.puzzle = puzzle;
    this.treeLevel = greedy ? 0 : (parent === undefined ? -1 : parent.treeLevel) + 1;
  ...
  genChilds(u, g, heuristic) {
    ...
    for (const dir in moves) {
      const newPuzzle = this.moveTile(zeroIdx, moves[dir], deepCopy(this.puzzle))
      if (newPuzzle) {
